...
# Modèles à essayer par ordre de priorité
...
async def generate_ai_audit(anomalies: list, filename: str, raw_text: str = "", images: list = None, career_data: list = None, **kwargs):
    ...
    Priorité Mistral avec Fallback Gemini (multi-modèles).
    """
...
- CONSIGNE VISION : Analyse très précisément chaque ligne du tableau de carrière.
- IDENTITÉ : Cherche le nom et le prénom de l'assuré.
- TABLEAUX : L'année est à gauche, l'employeur au milieu, les trimestres/points à droite.
- SALAIRES : Extrais le salaire brut annuel pour chaque année de carrière si disponible.
- POINTS : Extrais les points de retraite complémentaire."""
        ...
        for entry in career_data:
            ...
            if salary > 0:
                ...
                if abs(ris_pts - theo_pts) > 1.0:
                    expert_audit_notes.append(f"Année {year} : Écart détecté. Points RIS: {ris_pts} vs Théorie: {theo_pts}")
...
"""
        ...
        expert_context = f"""
**SYSTÈME DE RETRAITE :**
- Âge légal : {gen_params['legal_age_years']} ans.
- Trimestres requis : {gen_params['required_quarters']}.
"""

    base_prompt = f"""Tu es l'expert retraite de Hologram Conseils. Analyse ce Relevé Individuel de Situation (RIS) pour identifier précisément les anomalies ({filename}).
...
"""
    ...
    # 2. Backup : GEMINI (multi-modèles)
    if GEMINI_API_KEY:
        gemini_prompt = base_prompt + f"\n\n{vision_mode_desc if is_scan else ''}\n\nDonnées pré-analysées :\n{json.dumps(anomalies, ensure_ascii=False, indent=2)}\n\nTexte extrait :\n{raw_text}"
        for model_name in GEMINI_MODELS:
            try:
                logger.info(f"Tentative Gemini avec modèle : {model_name}")
                res = await _call_gemini(gemini_prompt, model_name, images)
                if is_valid_json(res):
                    return await _finalize_response(res)
            except Exception as e:
                logger.error(f"Erreur Gemini ({model_name}) : {str(e)}")
                time.sleep(1)

    # 3. Dernier recours : OLLAMA
    try:
    ...
    return res or json.dumps({"error": "Aucun service d'analyse disponible."})
...
async def _call_gemini(prompt: str, model_name: str, images: list = None):
